...
public class TableMenuAction 
	  extends AbstractMenuAction
{
	private static final Logger LOGGER = org.GeoRaptor.util.logging.Logging.getLogger("org.GeoRaptor.TableMenuAction");

    private static final String GENERAL_ERROR = MainSettings.EXTENSION_NAME + " Error";

	private static final String MENU_TABLE_COLUMN__ZOOM_TO_MAP             = "ZOOM_TO_MAP";
	private static final String MENU_TABLE_COLUMN__ADD_TO_MAP              = "ADD_TO_MAP";
	private static final String MENU_TABLE_COLUMN__METADATA_ARG            = "METADATA";
	private static final String MENU_TABLE_COLUMN__DROP_METADATA_ARG       = "DROP_METADATA";
	private static final String MENU_TABLE_COLUMN_CREATE_SPATIAL_INDEX_ARG = "CREATE_SPATIAL_INDEX";
	private static final String MENU_TABLE_COLUMN_DROP_SPATIAL_INDEX_ARG   = "DROP_SPATIA_INDEX";
	private static final String MENU_TABLE_COLUMN__EXPORT_COLUMN_ARG       = "EXPORT";
	  ...
	  public void launch()
	  {
	        ...
	        LOGGER.debug("Connection Type = " + connectionType + "; Schema = " + selectedSchemaName + "; Object = " + selectedObjectName + "; Column = " + selectedColumnName +"; Type = " + selectedObjectType);
			...
			try {
				...
				//debug: Action Name = raptor.objectaction.Oracle.VIEW_COLUMN.Manage Spatial Metadata....0c6d3cdc-4768-47ea-b9da-741c7539cb00
			} catch (Exception e) {}

			LOGGER.debug("Handling " + (menuType.equals("VIEW_COLUMN") ? "View Menu Entry" : "Materialized View Menu Entry") );
		
			if ( ! hasGeometryColumn(conn,
									 selectedSchemaName,
									 selectedObjectName,
									 selectedColumnName) )
			{
			...
			if ( menuArgument.equalsIgnoreCase(MENU_TABLE_COLUMN__METADATA_ARG) )
			{
				manageMetadata(conn,
			                   selectedSchemaName,
			                   selectedObjectName, 
			                   selectedColumnName,
			                   connectionUserName);
			} else if ( menuArgument.equalsIgnoreCase(MENU_TABLE_COLUMN__DROP_METADATA_ARG) )
			{
				dropMetadata(conn,
	                    selectedSchemaName,
	                    selectedObjectName, 
	                    selectedColumnName,
	                    connectionUserName);
			} else if ( menuArgument.equalsIgnoreCase(MENU_TABLE_COLUMN__ZOOM_TO_MAP) 
					    ||
					    menuArgument.equalsIgnoreCase(MENU_TABLE_COLUMN__ADD_TO_MAP) )
			{
				geometriesToMap(menuArgument,
								conn,
			...
			} else if ( menuArgument.equalsIgnoreCase(MENU_TABLE_COLUMN__EXPORT_COLUMN_ARG) ) 
			{
				export( conn, 
						selectedSchemaName,
		                selectedObjectName, 
			...
			} else if ( menuArgument.equalsIgnoreCase(MENU_TABLE_COLUMN_CREATE_SPATIAL_INDEX_ARG) )
			{
				createSpatialIndex( conn,
									connectionUserName,
									activeConnectionName,
									selectedSchemaName,
									selectedObjectName, 
									selectedColumnName);
			} else if (	menuArgument.equalsIgnoreCase(MENU_TABLE_COLUMN_DROP_SPATIAL_INDEX_ARG ) )
			{
...

	{
		...
		lrc = svp.addNewSpatialLayer(_selectedSchemaName, _selectedObjectName, _selectedColumnName, _selectedObjectType,
				_activeConnectionName, _conn, _menuArgument.equalsIgnoreCase(MENU_TABLE_COLUMN__ADD_TO_MAP) ? false : true);
